...
@app.command()
def move_device(group_name: str = typer.Argument(help="Group-name for devices to be moved"),config_file: str = typer.Argument(help="configuration file in json format ")):
  with open(config_file,'r') as config_file:
    ap_data = json.load(config_file)

  ap_sn = ap_data["serial"]
  module_resp = devices.move_devices(central,group_name,ap_sn)
  pprint(module_resp)

@app.command()
#def delete_device(ap_sn: str = typer.Argument(help="AP serial number ")):
def delete_device(config_file: str = typer.Argument(help="configuration file in json format ")):
  with open(config_file,'r') as config_file:
...
def show_gw_committed(group_name):
  ...
  with open(r"gw_config.json", 'w') as file:
